...
Nel pannello Impostazioni, ogni card ruolo mette in evidenza **Budget piano**, **Speso** e **Cashflow** (`pianoIniziale - speso`, col segno) come tre cifre grandi affiancate (non solo testo piccolo tra gli altri dettagli), con una barra di avanzamento sotto (verde entro il piano, rossa se lo sfora — stesso linguaggio visivo di `.rss-bar-fill` nella striscia di spesa del Listone). Il Cashflow è verde se positivo (margine ancora disponibile nel piano) e rosso se negativo (piano superato), stessa soglia della barra. I dettagli più tecnici (fattore reparto, budget "ribilanciato" — la disponibilità reale che tiene conto degli sfori sugli altri ruoli, vedi `computeRebalancedRoleTargets`) restano sotto, in testo più piccolo.
...
3. **Tabella giocatori** filtrabile per ruolo (tab P/D/C/A), con ricerca per nome/squadra, ordinabile per FVM, quotazione, media voto, rendimento, convenienza. Colonne: nome, squadra, quotazione (qa), FVM, media voto, rendimento (vedi punto 2bis, solo se le statistiche sono state importate — altrimenti "—"), convenienza (con indicatore colorato), range prezzo consigliato, azione "aggiungi alla rosa". Toggle **"Solo la mia rosa"** filtra la tabella ai soli giocatori già in "La mia rosa" per il ruolo attivo: permette di vedere le statistiche/colonne del Listone (quotazione, FVM, media, rendimento, convenienza) applicate ai propri giocatori già presi, invece di dover consultare "La mia rosa" (che non le mostra) o cercarli uno per uno.
...
4. **La mia rosa**: pannello/sezione con i giocatori selezionati raggruppati per ruolo, prezzo pagato (editabile, l'utente inserisce il prezzo reale di aggiudicazione in asta), possibilità di rimuovere. Ogni giocatore mostra anche il badge probabile titolare/non titolare (stesso `activeStartersMap()` usato per il pallino nel Listone, vedi punto 7), così si vede a colpo d'occhio se chi si è comprato gioca davvero. Il campo prezzo aggiorna le cifre di budget in alto **mentre si scrive** (evento `input`, non solo al `change`/blur) senza ricreare il campo mentre l'utente sta digitando — solo `renderDashboard`/`renderRoleSpendStrip` vengono richiamate lì, mai `renderRoster`/`renderAll` (altrimenti l'input perderebbe il focus a metà digitazione). Validazione e salvataggio restano sul `change`, invariati.

   **Archivio formazioni + esportazione PDF**: "Salva formazione nell'archivio" cattura una **fotografia indipendente** della rosa attuale (nome+squadra+ruolo+prezzo pagato per ogni giocatore, non gli Id del listone) con nome auto-generato da data/ora, rinominabile cliccandoci sopra. Resta nell'archivio anche se poi si svuota o si modifica "La mia rosa" — utile per confrontare versioni nel tempo o tenere un salvataggio "di sicurezza" a fine asta. Ogni formazione (archiviata o la rosa live tramite "Esporta PDF (rosa attuale)") si può esportare in PDF tramite la stampa nativa del browser: si popola un contenitore dedicato (`#print-formation`, nascosto normalmente) con una tabella per ruolo, poi `window.print()` — su desktop il dialogo di stampa offre "Salva come PDF", su iOS Safari lo stesso via Condividi → Stampa. Nessuna libreria di generazione PDF aggiunta: un `@media print` isola il contenitore dal resto dell'interfaccia (tema scuro non stampato, foglio bianco con testo nero).
...
App separata (`https://asta-live-fantacalcio-production.up.railway.app`, indirizzo/room code configurabili nel pannello "Asta Live") che gestisce l'asta in tempo reale. Collegamento in due parti, entrambe attivabili solo dall'admin della stanza:

1. **"La mia rosa" automatica** (esistente): scelta la propria squadra nella stanza, un polling ogni 2s (`SYNC_POLL_INTERVAL_MS`) su `GET /api/rooms/{roomCode}/sync/roster?participantId={teamId}` aggiunge in automatico a "La mia rosa" ogni giocatore risultato acquistato (nome, ruolo, squadra reale, prezzo pagato), abbinato al listone per nome+squadra normalizzati o creato come voce sintetica se non presente. Se l'admin disattiva la sincronizzazione per questa squadra o la stanza non esiste più, l'errore (403/404) ferma il polling e lo segnala in UI.
2. **Disponibilità dell'intero listone** (nuovo): mentre collegati, un secondo polling più diradato ogni 8s (`ROOM_SOLD_POLL_INTERVAL_MS` — non serve la stessa reattività del punto 1, e genera una richiesta per OGNI squadra della stanza quindi va tenuto più leggero) interroga `GET /api/rooms/{roomCode}/sync/teams` per l'elenco squadre, poi il roster di ciascuna in parallelo. Il risultato aggregato (`sync.soldByTeam`, chiave nome+squadra normalizzati → nome della squadra acquirente) marca come "venduti" tutti i giocatori già assegnati a **qualsiasi** squadra della stanza (non solo la propria). Nel Listone, un toggle "Nascondi venduti" (default attivo) li filtra dalla tabella; disattivato, restano visibili ma disabilitati con badge "venduto" (tooltip con la squadra acquirente) — stesso trattamento visivo della regola off-limits. Una riga di stato sopra la tabella mostra quante squadre della stanza rispondono correttamente (es. "2/3 squadre sincronizzate") e invita ad attivare la sincronizzazione anche per le altre se incompleta: **serve che l'admin la attivi per ogni squadra**, non solo per quella collegata dall'utente — una squadra non attivata risponde 403 su quell'unica richiesta e viene saltata senza bloccare le altre.

Entrambi i polling partono/si fermano insieme (stessa funzione `startPolling`/`stopPolling`, legata al collegamento della propria squadra) e i dati (`sync.soldByTeam` e relativi contatori) persistono in `localStorage` per restare visibili finché il polling successivo non li aggiorna.

### Navigazione a schede
Il menu in alto (`quick-nav`, centrato) funziona come schede vere e proprie: il contenitore principale mostra **sempre e solo** il pannello del pulsante attivo (`showPanel(id)` nasconde tutti gli altri con l'attributo `hidden`), non più uno scroll continuo tra tutti i pannelli — riduce lo scorrimento e rende più immediato passare da una sezione all'altra durante l'asta. Il pulsante del pannello corrente è evidenziato in oro (stesso linguaggio visivo dei tab ruolo `role-tab.active`). Un solo listener delegato intercetta il click su qualunque link interno con `href="#idPannello"` (sia i pulsanti del menu sia riferimenti incrociati dentro un pannello, es. "vedi pannello Probabili titolari" nel Listone) e richiama `showPanel`, azzerando lo scroll verticale. Il pannello attivo persiste in `localStorage` (chiave `fanta_asta_active_panel`, separata dallo stato applicativo principale) e viene ripristinato al refresh; un hash diretto nell'URL (es. apertura da un link/bookmark con `#roster-panel`) ha priorità e apre subito quel pannello. Default alla prima visita: "Listone".
